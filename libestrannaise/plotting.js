...
/**
 * Re-calculate all curves and rebuild plotting options
 ...
 */
export function plotCurveOptions(dataset, options = generatePlottingOptions()) {
    ...
    if(options.capMin)
        xMin = Math.max(xMin, options.capMin);

    if(options.capMax)
        xMax = Math.min(xMax, options.capMax);
...
export function plotCurves(dataset, options = generatePlottingOptions(), returnSVG = true) {
    let e2curve = Plot.plot(plotCurveOptions(dataset, options, returnSVG));
